...
class Module13 extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
  ...
  def comboSetsForSequences(sequences: List[String]): List[Set[Char]] = {
    ...
    val firstStrToLst = head.toList.map(x => Set(x))
    rest.foldLeft(firstStrToLst)(combineZippedSetsAndString(_, _))
  }
  ...
  def mysteryFunction(numbers: List[Int]): List[Int] = numbers.sorted
