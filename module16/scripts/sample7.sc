...
fCalc()
Await.ready(fCalc(), 1.second)

def resetTimer = time = 0

resetTimer  // back to not working
...
def retry[T](op: => T, retries: Int): Future[T] = {
  Future(op).
    recoverWith{ case NonFatal(_) if (retries > 0) => retry(op, retries - 1) }
}
...
Await.ready(f4, 1.second)

val timer = new Timer("terying", true)
